...
// Resources by Time (sums resources for raids arriving within the given time)
document.getElementById("analyzeResourcesByTime").addEventListener("click", () => {
  const timeRange = parseInt(document.getElementById("timeInputResources").value);
  if (isNaN(timeRange)) {
    document.getElementById("resourcesByTimeResult").innerText = "Please enter a valid time.";
    return;
  }
  getRaidData((raids) => {
    ...
    document.getElementById("resourcesByTimeResult").innerText = JSON.stringify(totalResources, null, 2);
  });
});
